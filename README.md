...
<small>RX671/Type 1YN TSIP のTLS 1.2 OTAは未成立です。従来構成はapplication約667,469 bytesとType 1YN blob 254,634 bytesの合計約922,103 bytesとなり、768 KiB（786,432 bytes）のinstall領域を約135,671 bytes超過するため、現行formal OTA profileは安全側にrejectします。Mbed TLS 3.6の小容量TLS 1.2構成でTSIP RSA CertificateVerifyを使用する実装と実機確認を [benchmark issue #25](https://gitlab.saffti.jp/oss/experiment/embedded/mcu/renesas/rx/example/ek-rx671/benchmark/tsip_mbedtls/-/issues/25) で進め、成功するまでは`—`を維持します。</small>
...
| RX671/Type 1YN AWS IoT | software / TSIP MQTTとFleetをTLS 1.2 / TLS 1.3でRPi#1実機Runner検証。software OTAはTLS 1.2 / TLS 1.3、TSIP OTAはTLS 1.3で、SCI6 runtime provisioning、`0.1.0`→`0.1.1`、self-test / image acceptance / Job `SUCCEEDED`、AWS cleanupまで成功し`○`。TSIP TLS 1.2 OTAは容量超過の解消を継続中。 |
