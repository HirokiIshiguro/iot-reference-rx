...
| 8 | CI/CD パイプライン統合 | In Progress (branch validated including OTA auto-run; merge pending) |
...
- `test_ota` は branch pipeline 上で **既定 auto 実行** に切り替え済み
...
2. merge 後の `main` pipeline で `build` / `flash` / `provision` / `test_mqtt` / `test_ota` の実行状態を再確認する
3. `test_ota` は auto-run を維持しつつ、しばらくは `allow_failure: true` のまま success 実績を積む
4. 採番済み pipeline / job / artifact の証跡を `CLAUDE.md` に追記し、再現可能な運用記録として残す

推奨実行順:

1. `codex/step8-pipeline-integration` を MR 化して merge する
2. merge 後の `main` pipeline を 1 回確認し、blocking 化された `provision` / `test_mqtt` と auto-run `test_ota` の結果を採番付きで記録する
3. `test_ota` は auto-run を継続し、既存メモどおり「追加 recovery なしで 3 回以上 success」を満たしたら `allow_failure: true` を外すか再検討する
...
- `claude-index/DEVELOPMENT.md` を参照し、follow-up 用 issue `#9`
  (`Step 8: auto-run OTA test on branch pipelines`) を起票
