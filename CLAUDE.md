...
- retry 順序:
  1. J14 と J20 が同じ board を見ていることを確認
  2. UART 復旧
  3. `test_mqtt.py` で CLI / MQTT baseline 復旧確認
  4. その後に provisioning / OTA Job 作成へ進む
...
- 停止点は `flash -> UART recovery` 境界に限定
  - 詳細ログと実行コマンドは `docs/ja/step7_4_ota_attempt.md`
  - 次の論点は issue #8 系の UART bridge / board power recovery
