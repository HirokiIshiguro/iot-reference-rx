...
| 7 | OTA テスト | In Progress (Step 7-1 to 7-3) |
...
- retry 順序:
  ...
  2. `OBE110020` で flash / `miniterm COM6 115200` を再試行
  3. それでも無音なら UART 復旧
  4. `test_mqtt.py` で CLI / MQTT baseline 復旧確認
  5. その後に provisioning / OTA Job 作成へ進む
...
- 停止点はいったん `correct E2 Lite selection -> flash -> UART observation` に修正
  ...
  - 次の論点は `OBE110020` での再試行。その後も無音なら issue #8 系の UART bridge / board power recovery
