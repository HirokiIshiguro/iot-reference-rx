        ...
        uint8_t *client_iv, uint8_t *server_iv)
{
    ...
    if (TSIP_SUCCESS == error_code)
    {
        ...
        {
            client_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA1_FOR_TLS;
            server_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA1_FOR_TLS;
            client_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES128_FOR_TLS;
            server_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES128_FOR_TLS;
        }
        else if (R_TSIP_TLS_RSA_WITH_AES_256_CBC_SHA == select_cipher_suite)
        {
            client_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA1_FOR_TLS;
            server_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA1_FOR_TLS;
            client_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES256_FOR_TLS;
            server_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES256_FOR_TLS;
        }
        else if (R_TSIP_TLS_RSA_WITH_AES_256_CBC_SHA256 == select_cipher_suite)
        {
            client_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA256_FOR_TLS;
            server_mac_key_index->type = TSIP_KEY_INDEX_TYPE_HMAC_SHA256_FOR_TLS;
            client_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES256_FOR_TLS;
            server_crypto_key_index->type = TSIP_KEY_INDEX_TYPE_AES256_FOR_TLS;
        }
        else
