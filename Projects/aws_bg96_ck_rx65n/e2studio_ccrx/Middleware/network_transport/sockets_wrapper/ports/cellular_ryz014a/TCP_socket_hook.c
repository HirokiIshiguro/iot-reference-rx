...
static BaseType_t prvLoadCellularApConfig( st_cellular_ap_cfg_t * p_ap_cfg );
...
bool Connect2AP( void )
{
    ...
    st_cellular_ap_cfg_t ap_cfg = { 0 };
    st_cellular_ap_cfg_t * p_ap_cfg = NULL;

    if( pdTRUE == prvLoadCellularApConfig( &ap_cfg ) )
    {
        p_ap_cfg = &ap_cfg;
        LogInfo( ( "Using APN from key store: %s (auth=%u)\r\n", ap_cfg.ap_name, ( unsigned int ) ap_cfg.auth_type ) );
    }
    else
    {
        LogInfo( ( "Using built-in APN settings.\r\n" ) );
    }
	do
	{
		countAP++;
		...
		if(CELLULAR_SUCCESS == ret )
		{
			...
			ret = R_CELLULAR_APConnect(&cellular_ctrl, p_ap_cfg);
		}
		else
		{
...
static BaseType_t prvLoadCellularApConfig( st_cellular_ap_cfg_t * p_ap_cfg )
{
    ...
    memset( p_ap_cfg, 0x00, sizeof( *p_ap_cfg ) );
    strncpy( ( char * ) p_ap_cfg->ap_name,
             CELLULAR_STRING_CONVERT( CELLULAR_CFG_AP_NAME ),
             sizeof( p_ap_cfg->ap_name ) - 1U );
    strncpy( ( char * ) p_ap_cfg->ap_user_name,
             CELLULAR_STRING_CONVERT( CELLULAR_CFG_AP_USERID ),
             sizeof( p_ap_cfg->ap_user_name ) - 1U );
    strncpy( ( char * ) p_ap_cfg->ap_pass,
             CELLULAR_STRING_CONVERT( CELLULAR_CFG_AP_PASSWORD ),
             sizeof( p_ap_cfg->ap_pass ) - 1U );
    p_ap_cfg->auth_type = CELLULAR_CFG_AUTH_TYPE;

    length = prvGetCacheEntryLength( KVS_CELLULAR_AP_NAME );
    if( length > 0U )
    {
        if( length >= sizeof( p_ap_cfg->ap_name ) )
        {
            length = sizeof( p_ap_cfg->ap_name ) - 1U;
        }
        xReadEntry( KVS_CELLULAR_AP_NAME, p_ap_cfg->ap_name, length );
        p_ap_cfg->ap_name[ length ] = '\0';
        loaded = pdTRUE;
    }

    length = prvGetCacheEntryLength( KVS_CELLULAR_AP_USERID );
    if( length > 0U )
    {
        if( length >= sizeof( p_ap_cfg->ap_user_name ) )
        {
            length = sizeof( p_ap_cfg->ap_user_name ) - 1U;
        }
        xReadEntry( KVS_CELLULAR_AP_USERID, p_ap_cfg->ap_user_name, length );
        p_ap_cfg->ap_user_name[ length ] = '\0';
        loaded = pdTRUE;
    }

    length = prvGetCacheEntryLength( KVS_CELLULAR_AP_PASSWORD );
    if( length > 0U )
    {
        if( length >= sizeof( p_ap_cfg->ap_pass ) )
        {
            length = sizeof( p_ap_cfg->ap_pass ) - 1U;
        }
        xReadEntry( KVS_CELLULAR_AP_PASSWORD, p_ap_cfg->ap_pass, length );
        p_ap_cfg->ap_pass[ length ] = '\0';
        loaded = pdTRUE;
    }

    length = prvGetCacheEntryLength( KVS_CELLULAR_AP_AUTH_TYPE );
    if( length > 0U )
    {
        if( length >= sizeof( auth_buff ) )
        {
            length = sizeof( auth_buff ) - 1U;
        }
        xReadEntry( KVS_CELLULAR_AP_AUTH_TYPE, auth_buff, length );
        auth_buff[ length ] = '\0';
        p_ap_cfg->auth_type = ( e_cellular_auth_type_t ) strtoul( auth_buff, NULL, 10 );
        loaded = pdTRUE;
    }
