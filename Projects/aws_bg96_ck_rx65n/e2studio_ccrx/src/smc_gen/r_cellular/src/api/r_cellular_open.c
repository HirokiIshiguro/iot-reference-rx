...
e_cellular_err_t R_CELLULAR_Open(st_cellular_ctrl_t * const p_ctrl, const st_cellular_cfg_t * const p_cfg)
{
    ...
    CELLULAR_LOG_INFO(("R_CELLULAR_Open: state gate start."));
    ...
    cellular_interrupt_enable(preemption);
    CELLULAR_LOG_INFO(("R_CELLULAR_Open: state gate complete."));
...
static e_cellular_err_t cellular_init(st_cellular_ctrl_t * const p_ctrl, const st_cellular_cfg_t * const p_cfg)
{
    ...
    if (CELLULAR_SEMAPHORE_SUCCESS == semaphore_ret)
    {
        ...
        if (CELLULAR_SUCCESS != ret)
        {
            CELLULAR_LOG_INFO(("First sending AT command to BG96 -> Failed."));
            goto R_CELLULAR_Open_fail;
        }

        CELLULAR_LOG_INFO(("First sending AT command to BG96 -> Succeeded."));

        /* BG96 has no equivalent of AT+SQNSIMST. SIM status is queried via AT+CPIN? below. */
