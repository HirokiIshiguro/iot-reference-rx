...
static e_cellular_err_t cellular_set_operator(st_cellular_ctrl_t * const p_ctrl, const uint8_t * const p_operator)
{
    ...
    /* BG96 has no equivalent of AT+SQNCTM (Sequans-only Conformance Test Mode).
     * Carrier profile on BG96 is determined by the SIM and firmware defaults.
     * The cbt-aqua prototype (working BG96 driver) does not perform any operator
     * / carrier-profile selection at init either — it goes ATE0 -> QICSGP ->
     * CGATT? -> QIACT directly. So this function is a no-op for BG96. The
     * caller R_CELLULAR_SetOperator() still runs atc_cfun_check / atc_cfun
     * after this returns, which BG96 supports natively.
     *
     * p_operator is intentionally unused; keep the parameter to preserve the
     * RYZ014A signature so callers (e.g. TCP_socket_hook.c Connect2AP) build
     * unchanged. */
    (void) p_ctrl;
    (void) p_operator;
    return CELLULAR_SUCCESS;
#else
    /* Original RYZ014A behavior: query SQNCTM, switch profile if differs,
     * then wait for module restart URC. */
