...
static void cellular_job_check(st_cellular_ctrl_t * p_ctrl, st_cellular_receive_t * cellular_receive)
{
    ...
    switch (p_cellular_receive->job_status)
    {
        case JOB_STATUS_NONE:
        {
            ...
            {
#if defined(CELLULAR_TARGET_BG96)
                /* BG96 has echo ON by default until ATE0 takes effect, so the receive
                 * buffer can hold the echoed command bytes (e.g. "ATE0\r") followed by
                 * the response "\r\nOK\r\n". The original RYZ014A path required the
                 * '\n' to land exactly at recv_count==2 (i.e. buffer is just "\r\n").
                 * Relaxed condition: any "\r\n" boundary is accepted. tmp_recvcnt is
                 * set to recv_count so subsequent slice parsing starts at the byte
                 * after the boundary. The whole-buffer strstr in cellular_response_check
                 * still matches "OK\r\n" / "ERROR\r\n" wherever they appear. */
                if ((p_cellular_receive->recv_count >= 2) &&
                    (CHAR_CHECK_7 ==
                     (char)p_ctrl->sci_ctrl.receive_buff[p_cellular_receive->recv_count - 2]))
                {
        ...
        case JOB_STATUS_COLON_CHECK:
        {
            ...
            {
#if defined(CELLULAR_TARGET_BG96)
                /* BG96 long-prefix URCs like '+QIURC: "dnsgip"' / '+QIURC:
                 * "recv"' don't carry the subtype yet at the first ':'
                 * (e.g. slice "+QIURC:"). Setting job_no=MAX here would
                 * redirect subsequent bytes to cellular_response_skip,
                 * which eats the URC on the next '\n' before the subtype
                 * is ever inspected. Instead, retry string_check at '\n'
                 * on the full slice and only commit the match if it
                 * succeeds. tmp_recvcnt must not advance in that case
                 * either, so the '\n'-time slice still contains the
                 * '+QIURC:' prefix for matching. */
                e_atc_return_code_t try = cellular_response_string_check(
                    &p_ctrl->sci_ctrl.receive_buff[p_cellular_receive->tmp_recvcnt]);
                if (CELLULAR_RES_MAX != try)
                {
                    p_cellular_receive->job_no = try;
                    p_cellular_receive->tmp_recvcnt = p_cellular_receive->recv_count;
                }
                /* else: stay in COLON_CHECK, retry match at '\n'. */
#else
...
static void cellular_response_check(st_cellular_ctrl_t * p_ctrl, st_cellular_receive_t * cellular_receive)
{
    ...
    if ((char)p_cellular_receive->data == (CHAR_CHECK_4))  //(uint8_t)->(char)
    {
        ...
        switch (p_cellular_receive->job_no)
        {
            ...
            {
#if defined(CELLULAR_TARGET_BG96)
                /* BG96: with echo ON during the very first ATE0, the buffer
                 * holds "ATE0\r\r\n" at this point and the trailing "OK\r\n"
                 * has not yet arrived. response_string_check returns
                 * CELLULAR_RES_MAX. The original RYZ014A flow would have left
                 * job_no=MAX, so the next byte ('O') is dispatched to
                 * cellular_response_skip — which then eats the OK on the next
                 * '\n'. Keep accumulating in CELLULAR_RES_CHECK state instead
                 * so the next '\n' (after "OK") triggers response_check again
                 * and finds "OK\r\n" via strstr against the whole buffer.
                 * Buffer overflow is bounded by the recv_task overflow guard. */
                if (CELLULAR_RES_MAX == p_cellular_receive->job_no)
                {
                    p_cellular_receive->job_no = CELLULAR_RES_CHECK;
                    return;
                }
...
static void cellular_get_data_reception(st_cellular_ctrl_t * p_ctrl, st_cellular_receive_t * cellular_receive)
{
    ...
    {
        const char * slice =
            (const char *)&p_ctrl->sci_ctrl.receive_buff[p_cellular_receive->tmp_recvcnt];
        const char * comma = strchr(slice, ',');
        if (NULL != comma)
        {
            int32_t socket_no = 0;
            int32_t data_len  = 0;
                                            ...
                                            const int32_t length)
{
    ...
    {
        uint8_t p_recv_null_log = 1;
        uint32_t total_log = 0;
        int16_t pending_log = 0;

        if (sidx < p_ctrl->creatable_socket)
        {
            p_recv_null_log = (NULL == p_ctrl->p_socket_ctrl[sidx].p_recv) ? 1U : 0U;
            total_log = p_ctrl->p_socket_ctrl[sidx].total_recv_count;
            pending_log = p_ctrl->p_socket_ctrl[sidx].receive_unprocessed_size;
        }

        CELLULAR_LOG_ERROR(("BG96 QIRD fast drain parameter error: sidx=%u creatable=%u length=%ld requested=%u p_recv_null=%u total=%lu pending=%d.\n",
                            (unsigned int)sidx,
                            (unsigned int)p_ctrl->creatable_socket,
                            (long)length,
                            (unsigned int)requested_size,
                            (unsigned int)p_recv_null_log,
                            (unsigned long)total_log,
                            (int)pending_log));
        ret = false;
    }
    ...
    if ((true == ret) && (length > 0))
    {
        ...
        {
            CELLULAR_LOG_ERROR(("BG96 QIRD payload read failed: sidx=%u length=%ld requested=%u total=%lu pending=%ld.\n",
                                (unsigned int)sidx,
                                (long)length,
                                (unsigned int)requested_size,
                                (unsigned long)p_ctrl->p_socket_ctrl[sidx].total_recv_count,
                                (long)pending_size));
        }
    }
    ...
    if (true == ret)
    {
        ...
        if (false == ret)
        {
            CELLULAR_LOG_ERROR(("BG96 QIRD trailer read failed: sidx=%u length=%ld requested=%u total=%lu pending=%ld.\n",
                                (unsigned int)sidx,
                                (long)length,
                                (unsigned int)requested_size,
                                (unsigned long)p_ctrl->p_socket_ctrl[sidx].total_recv_count,
...
static bool cellular_bg96_read_exact(st_cellular_ctrl_t * p_ctrl, uint8_t * p_dst, uint16_t length)
{
    ...
    while (copied < length)
    {
        ...
        if (SCI_SUCCESS != R_SCI_Control(p_ctrl->sci_ctrl.sci_hdl,
                                         ...
                                         &avail))
        {
            CELLULAR_LOG_ERROR(("BG96 QIRD read_exact queue check failed: length=%u copied=%u.\n",
                                (unsigned int)length,
                                (unsigned int)copied));
            ret = false;
            break;
...
static void cellular_dns_result(st_cellular_ctrl_t * p_ctrl, st_cellular_receive_t * cellular_receive)
{
    st_cellular_receive_t * p_cellular_receive = cellular_receive;
#if defined(CELLULAR_TARGET_BG96)
    /* BG96 emits two URCs per AT+QIDNSGIP:
     *   (1) +QIURC: "dnsgip",<err>,<count>,<ttl>   <- status only, no IP
     *   (2) +QIURC: "dnsgip","<ip>"                <- quoted IP string
     *
     * We land here at the trailing '\n' of each URC. The whole line is in
     * receive_buff. Extract the SECOND quoted token after "dnsgip" and
     * copy it (without quotes) to recv_data. Status-only URC (1) has no
     * second quoted field, so the copy is silently skipped. */
    if (CHAR_CHECK_4 == p_cellular_receive->data) /* '\n' */
    {
        if (NULL != p_ctrl->recv_data)
        {
            const char * buff  = (const char *)p_ctrl->sci_ctrl.receive_buff;
            const char * dns_q = strstr(buff, "\"dnsgip\"");
            if (NULL != dns_q)
            {
                /* Find the opening quote of the IP after "dnsgip" and ','. */
                const char * ip_start = strchr(dns_q + 8, '"'); /* 8 = strlen("\"dnsgip\"") */
                if (NULL != ip_start)
                {
                    ip_start++; /* skip opening quote */
                    const char * ip_end = strchr(ip_start, '"');
                    if ((NULL != ip_end) &&
                        ((size_t)(ip_end - ip_start) <= CELLULAR_DNS_LENGTH))
                    {
                        size_t iplen = (size_t)(ip_end - ip_start);
                        memcpy(p_ctrl->recv_data, ip_start, iplen);
                        ((char *)p_ctrl->recv_data)[iplen] = '\0';
                    }
                }
            }
        }
