...
e_cellular_err_t atc_sqnsrecv(st_cellular_ctrl_t * const p_ctrl, const uint8_t socket_no, const int32_t length)
{
    ...
    atc_generate(p_ctrl->sci_ctrl.atc_buff, gp_at_command[ATC_RECV_SOCKET], p_command_arg);

    /* BG96: record the socket index so the +QIRD: <length> response
     * handler can associate the length with the correct socket_ctrl entry.
     * AT+QIRD response format (unlike RYZ014A +SQNSRECV: <sock>,<len>)
     * carries only the length; the socket has to be remembered from the
     * outgoing request. */
    p_ctrl->sci_ctrl.active_recv_socket =
