...
{
    (void) vp_ctx;
    LogInfo( ( "ota_sha256_init_function: start" ) );

    if (pdFALSE == CRYPTO_SignatureVerificationStart( &s_ctx_iot,
                                                cryptoASYMMETRIC_ALGORITHM_ECDSA,
                                                cryptoHASH_ALGORITHM_SHA256 ) )
    {
        LogError( ( "ota_sha256_init_function: start failed" ) );
        return -1;
    }
    LogInfo( ( "ota_sha256_init_function: ready" ) );
    return 0;
}
/**********************************************************************************************************************
...
int32_t ota_sha256_final_function(uint8_t *p_hash, void * vp_ctx)
{
    (void) p_hash;
    (void) vp_ctx;
    LogInfo( ( "ota_sha256_final_function: done" ) );
    return 0;
}
/**********************************************************************************************************************
