 ...
 *********************************************************************************************************************/
/**********************************************************************************************************************
 * File Name    : bg96_private.h
 * Description  : Internal configuration settings for BG96 driver.
 *********************************************************************************************************************/
 ...
 * Macro definitions
 *********************************************************************************************************************/
#ifndef BG96_PRIVATE_H
#define BG96_PRIVATE_H

/* CELLULAR_TARGET_BG96
 *
 * Master switch for BG96-specific deviations from the Renesas r_cellular FIT
 * module's standard (RYZ014A) behavior. Defined to enable BG96 support; comment
 * out / undefine to restore the original FIT behavior. See the project tracking
 * issue for the full list of guarded deviations.
 *
 * Modified FIT sources include this header to gain access to the macro. */
#define CELLULAR_TARGET_BG96            (1)
...
#define BG96_ATC_ECHO_OFF                 "ATE0\r"
#define BG96_ATC_FUNCTION_LEVEL_CHECK     "AT+CFUN?\r"
#define BG96_ATC_FUNCTION_LEVEL           "AT+CFUN=%s\r"
#define BG96_ATC_PIN_LOCK_CHECK           "AT+CPIN?\r"
#define BG96_ATC_PIN_LOCK_RELEASE         "AT+CPIN=\"%s\"\r"
/* BG96 native socket AT commands (Quectel QI* family). These replace the
 * RYZ014A-style SQN* commands because BG96 does not understand the Sequans
 * dialect. Format args for each:
 *   QIOPEN:   %s=connectID, %s=service_type("TCP"|"UDP"), %s=host, %s=port
 *             (contextID fixed to 1, local_port=0, access_mode=0 buffer)
 *   QICLOSE:  %s=connectID
 *   QISEND:   %s=connectID, %s=send_length
 *   QIRD:     %s=connectID, %s=read_length
 *   QIDNSGIP: %s=hostname   (contextID fixed to 1) */
#define BG96_ATC_CONNECT_SOCKET           "AT+QIOPEN=1,%s,\"%s\",\"%s\",%s,0,0\r"
#define BG96_ATC_CLOSE_SOCKET             "AT+QICLOSE=%s\r"
#define BG96_ATC_SEND_SCOKET              "AT+QISEND=%s,%s\r"
#define BG96_ATC_RECV_SCOKET              "AT+QIRD=%s,%s\r"
#define BG96_ATC_DNS_LOOKUP               "AT+QIDNSGIP=1,\"%s\"\r"
/* BG96: APN config via QICSGP. Context 1, IPv4 (PDP type 1), APN, empty user/pass, PAP=1.
 * Original RYZ014A AT+CGDCONT kept here for reference:
 *   "AT+CGDCONT=1,\"IPV4V6\",\"%s\",,,,0,0,0,0,0,0,1,,0\r"
 */
...
#define BG96_ATC_SOCKET_CONFIG_2          "AT+SQNSCFGEXT=%s,1,0,0\r"
#define BG96_ATC_LISTENING_SOCKET         "AT+SQNSL=%s,%s,%s,0\r"
#define BG96_ATC_CONNECT_CHECK            "AT+CGATT?\r"
#define BG96_ATC_SET_CONNECT_STATUS       "AT+CGATT=%s\r"
#define BG96_ATC_SHUTDOWN                 "AT+SQNSSHDN\r"
#define BG96_ATC_GET_TIME                 "AT+CCLK?\r"
#define BG96_ATC_SET_TIME                 "AT+CCLK=\"%s/%s/%s,%s:%s:%s%s\"\r"
#define BG96_ATC_RESET                    "AT^RESET\r"
#define BG96_ATC_SET_NOTICE_LEVEL         "AT+CEREG=%s\r"
#define BG96_ATC_GET_NOTICE_LEVEL         "AT+CEREG?\r"
#define BG96_ATC_AUTO_CONNECT             "AT+SQNAUTOCONNECT=%s\r"
#define BG96_ATC_AUTO_CONNECT_CHECK       "AT+SQNAUTOCONNECT?\r"
#define BG96_ATC_SIM_ST_OFF               "AT+SQNSIMST=0\r"
#define BG96_ATC_GET_SERVICE_STATUS       "AT+COPS?\r"
#define BG96_ATC_SET_PROVIDER             "AT+COPS=%s,2,\"%s%s\"\r"
#define BG96_ATC_GET_PDN_STATE            "AT+CGACT?\r"
#define BG96_ATC_ACTIVATE_PDN             "AT+CGACT=1,%s\r"
#define BG96_ATC_GET_IPADDR               "AT+CGPADDR=1\r"
#define BG96_ATC_GET_PSM                  "AT+CPSMS?\r"
#define BG96_ATC_SET_PSM                  "AT+CPSMS=%s,,,\"%s\",\"%s\"\r"
#define BG96_ATC_GET_EDRX                 "AT+SQNEDRX?\r"
#define BG96_ATC_SET_EDRX                 "AT+SQNEDRX=%s,4,\"%s\",\"%s\"\r"
#define BG96_ATC_GET_SIGNAL_STRENGTH      "AT+CSQ\r"
#define BG96_ATC_GET_SW_REVISION          "AT+CGMR\r"
#define BG96_ATC_GET_SERIAL_NUMBER        "AT+CGSN\r"
#define BG96_ATC_GET_SVN                  "AT+CGSN=3\r"
#define BG96_ATC_GET_MODULE_NAME          "AT+CGMM\r"
#define BG96_ATC_GET_MAKER_NAME           "AT+CGMI\r"
#define BG96_ATC_GET_IMSI                 "AT+CIMI\r"
#define BG96_ATC_SEND_COMMAND_TO_SIM      "AT+CRSM=%s,%s,%s,%s,%s,\"%s\",\"%s\"\r"
#define BG96_ATC_SET_INTER_CONFIG         "AT+SQNIPSCFG=%s,%s\r"
#define BG96_ATC_SET_RING_CONFIG          "AT+SQNRICFG=%s,3,%s\r"
#define BG96_ATC_SET_PSM_CONFIG           "AT+SQNPSCFG=%s\r"
#define BG96_ATC_SET_IND_NOTIFY           "AT+CMER=3,0,0,%s,0,0,0\r"
#define BG96_ATC_GET_PHONE_NUM            "AT+CNUM\r"
#define BG96_ATC_GET_ICCID                "AT+SQNCCID?\r"
#define BG96_ATC_PING                     "AT+PING=\"%s\",%s,%s,%s,%s\r"
#define BG96_ATC_GET_CELLINFO             "AT+SQNMONI=%s\r"
#define BG96_ATC_SET_CTM                  "AT+SQNCTM=\"%s\"\r"
#define BG96_ATC_GET_CTM                  "AT+SQNCTM?\r"
#define BG96_ATC_SET_BAND                 "AT+SQNBANDSEL=0,\"%s\",\"%s\"\r"
#define BG96_ATC_FACTORYRESET             "AT+SQNSFACTORYRESET\r"
#define BG96_ATC_SMCWRX                   "AT+SMCWRX=%s\r"
#define BG96_ATC_SMCWTX                   "AT+SMCWTX=%s,%s,%s\r"
#define BG96_ATC_CGPIAF                   "AT+CGPIAF=1,0,1,0\r"
#define BG96_ATC_CEER                     "AT+CEER\r"
#define BG96_ATC_FIRMUPGRADE              "AT+SQNSUPGRADE=\"%s\",%s,5,%s\r"
#define BG96_ATC_FIRMUPGRADE_SSL          "AT+SQNSUPGRADE=\"%s\",%s,5,%s,%s\r"
#define BG96_ATC_FIRMUPGRADE_CHECK        "AT+SQNSUPGRADE?\r"
#define BG96_ATC_GET_LR_SVN               "ATI1\r"
#define BG96_ATC_WRITE_CERTIFICATE        "AT+SQNSNVW=\"%s\",%s,%s\r"
#define BG96_ATC_ERASE_CERTIFICATE        "AT+SQNSNVW=\"%s\",%s,0\r"
#define BG96_ATC_GET_CERTIFICATE          "AT+SQNSNVR=\"%s\",%s\r"
#define BG96_ATC_CONFIG_SSL_PROFILE       "AT+SQNSPCFG=%s,2,,%s,%s,%s,%s,\"\"\r"
#if (CELLULAR_IMPLEMENT_TYPE == 'B')
#define BG96_ATC_CONFIG_SSL_SOCKET        "AT+SQNSSCFG=%s,%s,%s\r"
#endif
#define BG96_NO_COMMAND                   "\r"
