...
e_cellular_err_t R_CELLULAR_SetBand(st_cellular_ctrl_t * const p_ctrl, const uint8_t * const p_band)
{
    ...
    if (CELLULAR_SUCCESS == ret)
    {
        ...
        {
#if defined(CELLULAR_TARGET_BG96)
            /* BG96: AT+SQNCTM (Sequans Conformance Test Mode) and AT+SQNBANDSEL
             * are not supported. Band selection on BG96 is done via
             * AT+QCFG="band",... but is not required for SIM-driven operation —
             * the cbt-aqua prototype runs without any band configuration and
             * connects to LTE-M / NB-IoT successfully. p_band is intentionally
             * unused; the parameter is preserved so the caller
             * (TCP_socket_hook.c::Connect2AP) builds unchanged. The trailing
             * atc_cfun_check / atc_cfun(LEVEL4) below still runs and is
             * supported by BG96 natively. */
            (void) p_band;
