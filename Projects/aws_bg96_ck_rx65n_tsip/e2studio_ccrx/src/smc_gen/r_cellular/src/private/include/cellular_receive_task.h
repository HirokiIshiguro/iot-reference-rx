...
/* BG96 URC mappings (Quectel QI* family). strstr-based match in the receive
 * task tolerates the trailing subtype + cid + newline. The existing URC
 * dispatch handlers (cellular_get_data_reception, cellular_request_data,
 * cellular_dns_result, cellular_disconnect_socket) are format-parse
 * sensitive and are adjusted in Phase B follow-ups. For Phase A we accept
 * that non-latent MQTT data may require polling if the handler parse
 * mismatches BG96 format. */
#define ATC_RES_DATA_RECEIVE        "+QIURC: \"recv\""
#define ATC_RES_DATA_RECEIVE_QIRD   "+QIRD:"
 ...
 * pattern here keeps SYSTEM_START effectively disabled for BG96 — substring
 * matching against actual BG96 boot URCs ("RDY\r\n", "APP RDY\r\n") would
 * otherwise fire on the next AT response and cause the buffer to be cleared
 * mid-response, dropping the trailing "OK\r\n". */
